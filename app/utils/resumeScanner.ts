...
// Pull the real API key from your environment variables
const OCR_SPACE_API_KEY = process.env.NEXT_PUBLIC_OCR_API_KEY || 'helloworld';
...
export async function getResumeTextFromFirestore(): Promise<string | null> {
  ...
  try {
    ...
    // --- THE FIX: Explicitly tell OCR.space that this is a PDF ---
    ...
    if (apiData.IsErroredOnProcessing) {
      ...
      console.log('Resume text extracted:', extractedText.substring(0, 200) + '...'); 
